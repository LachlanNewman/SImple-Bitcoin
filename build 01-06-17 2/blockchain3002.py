...
#Karl Chen Gan
#Saves a copy of the current blockchain to a folder in the program's directory  
def saveBlockChain(blockchain):
    print("Saving the CURRENT version of the blockchain to the 'blockchain' folder in the program's directory.\n")

    #Create the blockchain folder if it doens't already exist
    blockchainFolder = os.path.join(os.getcwd(), r'blockchain')
    if not os.path.exists(blockchainFolder):
        os.mkdir(blockchainFolder)
        
    #Save each block
    height = 0
    for b in blockchain:
        with open("blockchain/block" + str(height) + ".json", "w") as fp:
            #Remove old contents
            fp.seek(0)
            fp.truncate()
