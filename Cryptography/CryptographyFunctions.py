...
def genRSAKeyPairs(ID):
    ...
    subprocess.call(["openssl", "genrsa", "-out", "private" + ID + ".pem", "2048"])
    
    #Write public key out to new file named publicX.pem
    subprocess.call(["openssl", "rsa", "-in", "private" + ID + ".pem", "-outform", "PEM", "-pubout", "-out", "public" + ID + ".pem"])    
...
def buildTransactionDict(srcID, destID, transaction):
    ...
    subprocess.call(["rm", "unsignedMessage.json"])    #Remove the unsigned message in preparation for next message
    ...
    subprocess.call(["rm", "sigBin"])    #Remove the binary representation of the signature
    ...
    subprocess.call(["rm", "sig64"])    #Remove the base64 representation of the signature
